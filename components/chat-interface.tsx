    ...
    await withLoading(async () => {
      try {
        ...
        const prefer = /\b(article|अनुच्छेद)\b/.test(q)
          ? "constitution"
          : /(\bv\.?s?\.?\b| vs |citation|holding|ratio|judgment)/i.test(
              currentInput
            )
          ? "cases"
          : undefined;
        ...
        setMessages((prev) => {
          ...
          try {
            const title =
              next.find((m) => m.sender === "user")?.content?.slice(0, 50) ||
              "New Chat";
            ...
            if (existing) {
              updated = threads.map((t) =>
                t.id === convId ? { ...t, title, lastMessage, updatedAt } : t
              );
            } else {
              updated = [
                { id: convId, title, lastMessage, updatedAt },
                ...threads,
              ];
            }
            setThreads(updated);
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {threads.length === 0 ? (
                <div className="mt-8 text-center text-sm text-muted-foreground">
                  <p>
                    {language === "en" ? "No chats yet." : "अभी कोई चैट नहीं।"}
                  </p>
                  <p>
                    {language === "en"
            ...
            <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
              <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
                ...
                <div className="text-xs text-muted-foreground">
                  {language === "en"
                    ? "Grounded in Constitution + Cases"
                    : "संविधान + नज़ीर आधारित"}
                </div>
              </div>
            </header>
                        ...
                        <Input
                          ...
                          placeholder={
                            language === "en"
                              ...
                              : "भारतीय कानून के बारे में पूछें…"
                          }
